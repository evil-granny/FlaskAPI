...
class Unpacker(object):
    ...
    def _unpack(self, execute=EX_CONSTRUCT):
        ...
        if execute == EX_READ_MAP_HEADER:
            ...
            return n
        # EstateItem should we eliminate the recursion?
        if typ == TYPE_ARRAY:
            if execute == EX_SKIP:
                for i in xrange(n):
                    # EstateItem check whether we need to call `list_hook`
            ...
            # EstateItem is the interaction between `list_hook` and `use_list` ok?
            return ret if self._use_list else tuple(ret)
        if typ == TYPE_MAP:
            if execute == EX_SKIP:
                for i in xrange(n):
                    # EstateItem check whether we need to call hooks
                    self._unpack(EX_SKIP)
                    self._unpack(EX_SKIP)
                return
